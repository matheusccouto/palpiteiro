...
ARTIFACT_FEATURES_PATH = os.path.join(THIS_DIR, "train", "{position}_features.json")

# Google Cloud Storage
BUCKET_NAME = os.environ["BUCKET_NAME"]
POSITIONS = ["goalkeeper", "defender", "fullback", "midfielder", "forward"]
MODEL_PATH = "points/{position}.joblib"
MODEL_FEATURES_PATH = "points/{position}_features.json"
...
def upload():
    ...
    for pos in POSITIONS:
        ...
        blob = bucket.blob(MODEL_FEATURES_PATH.format(position=pos))
        blob.upload_from_filename(ARTIFACT_FEATURES_PATH.format(position=pos))
