...
def handler(event, context=None):  # pylint: disable=unused-argument
    ...
    for match in data:
        ...
        row = {
            ...
            "max_home": np.max(list(odds["home"].values()), inital=np.nan),
            "max_away": np.max(list(odds["away"].values()), inital=np.nan),
            "max_draw": np.max(list(odds["draw"].values()), inital=np.nan),
